...
data["sentence"] = []
data["sentiment"] = []
for file_path in tqdm(os.listdir('/home/xiaoguzai/数据/aclImdb/train/pos'),desc=os.path.basename('/home/xiaoguzai/origin-code/aclImdb/train/pos')):
    with tf.io.gfile.GFile(os.path.join('/home/xiaoguzai/数据/aclImdb/train/pos',file_path),"r") as f:
...
data["sentence"] = []
data["sentiment"] = []
for file_path in tqdm(os.listdir('/home/xiaoguzai/数据/aclImdb/train/neg'),desc=os.path.basename('/home/xiaoguzai/origin-code/aclImdb/train/neg')):
    with tf.io.gfile.GFile(os.path.join('/home/xiaoguzai/数据/aclImdb/train/neg',file_path),"r") as f:
        data["sentence"].append(f.read())
        data["sentiment"].append(re.match("\d+_(\d+)\.txt", file_path).group(1))
...
data["sentence"] = []
data["sentiment"] = []
for file_path in tqdm(os.listdir('/home/xiaoguzai/数据/aclImdb/test/pos'),desc=os.path.basename('/home/xiaoguzai/origin-code/aclImdb/test/pos')):
    with tf.io.gfile.GFile(os.path.join('/home/xiaoguzai/数据/aclImdb/test/pos',file_path),"r") as f:
        data["sentence"].append(f.read())
        data["sentiment"].append(re.match("\d+_(\d+)\.txt", file_path).group(1))
        #data["sentence"]存储原文，比如0_9.txt,则后面对应的情感态度内容就为9
pos_df = pd.DataFrame.from_dict(data)
...
data["sentence"] = []
data["sentiment"] = []
for file_path in tqdm(os.listdir('/home/xiaoguzai/数据/aclImdb/test/neg'),desc=os.path.basename('/home/xiaoguzai/origin-code/aclImdb/test/neg')):
    with tf.io.gfile.GFile(os.path.join('/home/xiaoguzai/数据/aclImdb/test/neg',file_path),"r") as f:
        data["sentence"].append(f.read())
        data["sentiment"].append(re.match("\d+_(\d+)\.txt", file_path).group(1))
neg_df = pd.DataFrame.from_dict(data)
pos_df["polarity"] = 1
...
#它这里由于model之中申请好了最大的长度为128，所以这里将train_x长度不够128的补齐128的长度
#也就是这里train_x长度全部为128的长度
